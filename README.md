...
- 헤더 홈 버튼 그림(`public/media/brand-santa-<해시>.webp`) — 사이트 주인이 준 산타 사진(`santa2.jpg`, 출처 미확인)에서
  `scripts/prepare-brand-santa.py` 로 흰 배경을 지우고 높이 128px 로 줄인 것. 파일명의 해시는 내용에서 나오므로 그림을
  바꾸면 이름이 바뀐다 — `src/site.ts` 의 `brandMark` 도 같이 고친다. 사진의 저작권·이용 조건은 주인이 확인한다.
